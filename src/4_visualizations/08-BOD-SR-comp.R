...
d <- d %>% filter(!is.na(ci.lb), grepl("Pooled", country), 
                  analysis=="primary" | analysis=="primary-multi"| analysis=="SRMA comp"| analysis=="secondary POU"| analysis=="secondary",
                  adjusted==1, subgroup=="unstratified",
                  Y=="Diarrhea")

unique(BOD_data$X)

d[d$X=='POU_filter_V_imp_off_prem',] %>% filter(!is.na(X))
...
d$contrast[d$X=='POU_chlorine']
d$ref[d$X=='POU_chlorine']

unique(d$contrast[d$X=='san_imp_cat'])
unique(d$ref[d$X=='san_imp_cat'])

unique(d$contrast[d$X=='san_imp'])
unique(d$ref[d$X=='san_imp'])

#Secondary contrasts for the SR
d_sec <- readRDS(here("results/SRMA_comp_adjusted_RR.rds"))
unique(d_sec$X)
unique(d_sec$ref)
unique(d_sec$contrast)

unique(d$contrast[d$X=='piped_V_unimp'])
unique(d$ref[d$X=='piped_V_unimp'])
...
>>>>>>> a1fd13ecbd5d3ccdd82e54285f5d28942e809714
head(d)
head(BOD_data)
...
<<<<<<< HEAD
d <-  left_join(BOD_data %>% select(X, ref, contrast), d, by=c("X","ref","contrast"))
...
#Look at geographic locations on the SR's and do a subsetting analysis
#Need to check the highlighted in excel sheet to check if right coding for ref or contrast
=======
